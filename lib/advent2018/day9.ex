defmodule Day9 do
  # Cycle from left to right, leaving 7 available if we can
  #def cycle_left_right([one, two, three, four, five, six, seven | tail]) do
    #[[seven, six, five, four, three, two, one], tail]
  #end
  #def cycle_left_right(rev_left) do
    #[[], rev_left]
  #end
  ...
  defp _high_score(players, max_num, next_num, left, right, scores) when rem(next_num, 23) == 0 do
    ...
    [one, two, three, four, five, six, seven | left_tail] = left
    new_right = [six, five, four, three, two, one | right]

    new_scores = Map.put(scores, current_player, current_player_score + seven + next_num)
    _high_score(players, max_num, next_num + 1, left_tail, new_right, new_scores)
  end
